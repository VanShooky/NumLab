...
public class SLAE {
    ...
    public static void forwardElimination(Double[][] matrixA, Double[] matrixB){
        for (int i = 0; i < matrixA.length; i++) {
            int max = i;
             
            for (int j = i + 1; j < matrixA.length; j++) {
                if (Math.abs(matrixA[j][i]) > Math.abs(matrixA[max][i])) {
                    max = j;
                }
            }
            ...
            for (int j = i + 1; j < matrixA.length; j++) {
                ...
                matrixB[j] -= alpha * matrixB[i];
                for (int k = i; k < matrixA.length; k++) {
    ...
    public static void backSubstitution(Double[][] matrixA, Double[] matrixB, Double[] matrixX) {
    ...
    public static void swap (Double [][] matrixA, int i, int j){
        Double[] temp = matrixA[i];
        matrixA[i] = matrixA[j];
        matrixA[j] = temp;
    }

    public static void swap (Double [] matrixB, int i, int j){
        Double temp = matrixB[i];
        matrixB[i] = matrixB[j];
        matrixB[j] = temp;
    }

    public static Double[] residualVector(Double[][] matrixA1, Double[] matrixB1, Double[] matrixX){
    ...
    public static Double[] FactorizationMethod(Double[][] matrixA, Double[] matrixB) {
        ...
        fill(matrixA1,matrix_L,matrixD,matrixY,matrixZ);
    ...
    public static void fill(Double[][] matrixA1, Double[][]matrix_L, Double[][] matrixD, Double[] matrixY, Double[] matrixZ){
        for (int i = 0; i < matrixA1.length; i++) {
            for (int j = 0; j < matrixA1.length; j++) {
                if (i == j) {
                    matrix_L[i][j] = 1.0;
